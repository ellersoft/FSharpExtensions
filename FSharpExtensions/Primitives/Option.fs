...
/// Binds `inp` to `None -> v | Some inp -> inp`. Similar to `Option.bind`, but returns the passed value if the option is `None`, and the option value if `Some`.
let bindNone v = function | None -> v | Some inp -> inp
/// Maps `inp` to `None -> Some v | Some inp -> inp`. Similar to `Option.map`, but returns the passed value if the option is `None`, and the option value if `Some`.
